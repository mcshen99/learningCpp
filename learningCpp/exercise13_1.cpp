...
public:
	virtual ~Core() { }
	Core() : midterm(0), final(0) { cerr << "Core::Core()" << endl; }
	Core(std::istream& is) { read(is); cerr << "Core::Core(istream&)" << endl; }
...
public:
	Grad() : thesis(0) { cerr << "Grad::Grad()" << endl; }
	Grad(std::istream& is) { read(is); cerr << "Grad::Grad(istream&)" << endl; }
